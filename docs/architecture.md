...
The optional encrypted BLE edge service receives bounded samples through the existing event queue. `EdgeSwitch` runs only in the application loop. The companion never selects a slot directly: The first outward USB motion at the reported edge triggers switching. Only the source needs a companion; the destination needs a ready HID connection. The destination pointer is not repositioned. Samples carry an epoch and expire after 600 ms. Reconnection, configuration, and selection changes invalidate them.
