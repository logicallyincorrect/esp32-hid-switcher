...
public:
  ...
  bool motion(int16_t dx,uint8_t buttons,uint32_t received,uint32_t now){
    if(buttons){if(target>=0)reset(now);return false;}
    if(target>=0)return true;
    auto &host=_hosts[selected];
    if(!_allowed||!fresh(selected,now)||!host.armed||!host.sample.edge||uint32_t(now-_changed)<cooldown||uint32_t(now-received)>100)return false;
