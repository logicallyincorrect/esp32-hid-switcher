...
/**
 * @brief Get HID Interface pointer by owning device and endpoint address
 *
 * @param[in] ep_addr      Endpoint address
 * @return hid_iface_t     Pointer to HID Interface configuration structure
 */
static hid_iface_t *get_interface_by_ep(hid_device_t *device, uint8_t ep_addr) {
  hid_iface_t *interface = NULL;

  HID_ENTER_CRITICAL();
  STAILQ_FOREACH(interface, &s_hid_driver->hid_ifaces_tailq, tailq_entry) {
    if (device == interface->parent && ep_addr == interface->ep_in) {
      HID_EXIT_CRITICAL();
      return interface;
    }
  }
...
static void in_xfer_done(usb_transfer_t *in_xfer) {
  ...
  hid_iface_t *iface = get_interface_by_ep(get_hid_device_from_context(in_xfer), in_xfer->bEndpointAddress);
  assert(iface);
...
esp_err_t hid_host_device_get_params(hid_host_device_handle_t hid_dev_handle,
  ...
  if (usb_host_get_device_descriptor(iface->parent->dev_hdl, &descriptor) == ESP_OK) {
    dev_params->vid = descriptor->idVendor;
    dev_params->pid = descriptor->idProduct;
  }
  return ESP_OK;
}
